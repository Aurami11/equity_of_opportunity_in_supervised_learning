...
def _cli() -> None:
    ...
    p = argparse.ArgumentParser(description="Hardt et al. (2016) fairness post-processing (equalized odds / equal opportunity).")
    p.add_argument("--input", type=str, required=True, help="CSV path")
    p.add_argument("--label", type=str, required=True, help="Name of label column (binary 0/1)")
    p.add_argument("--group", type=str, required=True, help="Name of sensitive attribute column (group)")
    p.add_argument("--score", type=str, default=None, help="Name of score column (float). If provided, uses score post-processing.")
    p.add_argument("--pred", type=str, default=None, help="Name of prediction column (binary 0/1). If provided, uses binary post-processing.")
    p.add_argument("--method", type=str, choices=["equalized_odds", "equal_opportunity"], default="equalized_odds")
    p.add_argument("--cost_fp", type=float, default=1.0)
    p.add_argument("--cost_fn", type=float, default=1.0)
    p.add_argument("--n_thresholds", type=int, default=200)
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--output", type=str, default="fair_predictions.csv", help="Output CSV filename (written next to input).")

    args = p.parse_args()

    if (args.score is None) == (args.pred is None):
        raise SystemExit("Provide exactly one of --score or --pred.")

    df = pd.read_csv(args.input)
    y = df[args.label].to_numpy()
    a = df[args.group].to_numpy()

    if args.score is not None:
        s = df[args.score].to_numpy()
        pp = ScorePostProcessor(method=args.method, cost_fp=args.cost_fp, cost_fn=args.cost_fn, n_thresholds=args.n_thresholds, random_state=args.seed).fit(y, s, a)
        df["y_fair_proba"] = pp.predict_proba(s, a)
        df["y_fair"] = pp.predict(s, a)
        print("Learned mixtures:", pp.explain())
    else:
        yp = df[args.pred].to_numpy()
        pp = BinaryPostProcessor(method=args.method, cost_fp=args.cost_fp, cost_fn=args.cost_fn, random_state=args.seed).fit(y, yp, a)
        df["y_fair_proba"] = pp.predict_proba(yp, a)
        df["y_fair"] = pp.predict(yp, a)
        print("Learned flip probabilities:", pp.explain())

    out_path = args.output
    if not out_path.lower().endswith(".csv"):
        out_path += ".csv"
    # If output path is not absolute, place next to input
    if not (out_path.startswith("/") or out_path.startswith("./") or out_path.startswith("../")):
        from pathlib import Path
        out_path = str(Path(args.input).resolve().parent / out_path)

    df.to_csv(out_path, index=False)
    print(f"Saved: {out_path}")
    print("\nBefore/After report:")
    if args.score is not None:
        # baseline: threshold at 0.5 for probabilities in [0,1], else median threshold
        baseline = (s > np.median(s)).astype(int)
        print(fairness_report(y, baseline, a))
        print(fairness_report(y, df["y_fair"].to_numpy(), a))
    else:
        print(fairness_report(y, yp, a))
        print(fairness_report(y, df["y_fair"].to_numpy(), a))
